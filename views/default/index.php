...
$ids = [];
foreach ($manifest as $tag => $data) {
    if (!in_array($data['id'], array_keys($ids))) {
        $ids[$data['id']] = 2;
        continue;
    }
    $ids[$data['id']] = $ids[$data['id']] + 1;
}
?>
	...
	<div class="container-fluid">
		<div class="row-fluid">
			...
			<table class="table table-condensed table-bordered table-striped table-hover table-filtered" style="table-layout: fixed;">
				...
				<tbody>
                    <?php foreach ($ids as $key => $count) : ?>
                        <tr>
                        <th rowspan="<?php echo $count ?>"> <?php echo $key ?></th>
                        </tr>
				        <?php foreach ($manifest as $tag => $data): ?>
                            <?php if ($data['id'] !== $key) continue; ?>
                            <tr>
                            ...
                            <td><?php
                                if ($data['ajax'] === true) { echo "AJAX\n"; }
                                if ($data['code'] === 302) { echo "REDIRECT\n"; }
                                ?></td>
